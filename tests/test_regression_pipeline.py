...
class RegressionPipelineTests(unittest.TestCase):
    ...
    def _assert_selected_fields_rounded_to_4(self, obj: object) -> None:
        keys = {
            "fee_rate_pct",
            "ma_sum_prev",
            "threshold_from_ma",
            ...
            if isinstance(node, dict):
                for k, v in node.items():
                    if k in keys and isinstance(v, (int, float)) and not isinstance(v, bool):
                        self.assertEqual(v, round(float(v), 4), f"{k} must be rounded to 4 decimals")
                    walk(v)
                return
    ...
    def test_premarket_pipeline_matches_golden_outputs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self._assert_selected_fields_rounded_to_4(json.loads(actual_states))
