...
def _default_log_path(script_name: str='generate_report') -> str:
    ts = datetime.now().strftime('%Y%m%d_%H%M%S')
    pid = os.getpid()
    return str(Path('logs') / f'{script_name}_{ts}_{pid}.log')

def _enable_log_tee(log_path: str):
    p = Path(log_path)
...
def _disable_log_tee(fh, old_stdout, old_stderr) -> None:
    try:
        sys.stdout.flush()
        sys.stderr.flush()
    finally:
...
def report_title_from_meta(states: Dict[str, Any], mode: str) -> str:
    ...
    doc = meta.get('doc') or 'Daily Investment Report'
...
def render_report(states: Dict[str, Any], schema: Dict[str, Any], mode: str) -> str:
    ...
    meta = _effective_meta(states, mode)
    if not meta.get('report_context'):
...
def main() -> None:
    ...
    ap.add_argument('--schema', default='report_spec.json', help='report_schema.md or report_spec.json')
    ap.add_argument('--mode', required=True, help='Required mode snapshot to render, e.g. Premarket / Intraday / AfterClose')
    ap.add_argument('--date', default='', help='YYYY-MM-DD for filename only (optional)')
    ap.add_argument('--out', default='', help='output report md file path (optional)')
    ap.add_argument('--out-dir', default='.', help='output directory if --out not set')
    ap.add_argument('--log-file', default='', help='Write a detailed render log here. Default: logs/generate_report_<timestamp>_<pid>.log')
    args = ap.parse_args()
    log_target = str(args.log_file or '').strip() or _default_log_path('generate_report')
    log_path, fh, old_stdout, old_stderr = _enable_log_tee(log_target)
    try:
        _log_run_header('generate_report.py', args, log_path)
        states = json.loads(Path(args.states).read_text(encoding='utf-8'))
        _migrate_state_schema(states)
        schema = load_schema(args.schema)
        d = args.date.strip() or report_date_default(states, args.mode)
        if args.out.strip():
            out_path = args.out.strip()
        else:
            out_path = str(Path(args.out_dir) / f'{d}_{_normalize_mode_key(args.mode)}.md')
        md = render_report(states, schema, args.mode)
