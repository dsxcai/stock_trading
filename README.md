...
When `update_states.py` is run with `--mode`, it now attempts an automatic CSV refresh first. The refresh scope is limited to functionally active tickers: current holdings, strategy tickers, and configured FX pairs such as `TWD=X`. The mode update flow refreshes those active tickers every time instead of trusting the last CSV row to already be a finalized close. In `Premarket`, equities still refresh only through the latest completed NYSE trading day. In `Intraday`, equities refresh through the current ET date so same-day CSV prices can actually be used when available. Configured FX pairs are also allowed to refresh through the current ET date.
